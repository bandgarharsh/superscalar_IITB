...
architecture Behavioral of Branch_Execution_Unit is
    -- Internal signals for the X-Ray to read!
    ...
    begin
        ...
        if issue_valid = '1' then
            ...
            if issue_packet.op = OP_BEQ then
                ...
                v_actual_target := unsigned(issue_packet.pc) + 2; -- Default sequential
                v_actual_taken  := '0';
            end if;
            ...
            -- STEP 3: BROADCAST (For JAL/JLR saving return address)
    ...
    begin
        if rising_edge(clk) then
            if rst = '0' and issue_valid = '1' then
                write(l, string'("[BRANCH EXEC] Calculated PC: "));
                write(l, integer'image(to_integer(unsigned(issue_packet.pc))));
                
                if internal_mispred = '1' then
                    write(l, string'(" -> MISPREDICTED! Sending correction to ROB -> "));
                    write(l, integer'image(to_integer(unsigned(internal_target))));
                else
