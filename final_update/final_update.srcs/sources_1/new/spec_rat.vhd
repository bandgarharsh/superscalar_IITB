...
----------------------------------------------------------------------------------
-- MODULE: Speculative Register Alias Table (RAT)
-- DESCRIPTION:
--   Maintains the active mapping between 8 architectural registers and their 
--   most recent physical register assignments. Integrates combinational bypass 
--   logic to allow Slot 2 instructions to instantaneously reference physical 
--   registers assigned to Slot 1 in the exact same cycle (Intra-cycle RAW handling).
----------------------------------------------------------------------------------
entity spec_rat is
    Port (
        clk             : in  std_logic;
...
begin
    ...
    -- Slot 2 Lookups with Combinational Intra-Cycle Bypassing
    -- Resolves RAW dependencies if Slot 2 attempts to read what Slot 1 is actively renaming.
    ...
    begin
        if rst = '1' then
            -- Base physical-to-architectural identity mapping
            for i in 0 to 7 loop rat_map(i) <= std_logic_vector(to_unsigned(i, 5)); end loop;
            rat_flag <= (others => '0');
        elsif rising_edge(clk) then
            if flush = '1' then
                -- Complete alias overwrite from Retirement RAT (RRAT) on mispredict
                ...
                -- Write newly allocated physical registers back into architectural slots
