...
----------------------------------------------------------------------------------
-- MODULE: Register Rename & Dispatch Stage
-- DESCRIPTION:
--   Acts as the central router for the superscalar pipeline. 
--   1. Requests physical registers from the Free List.
--   2. Renames architectural registers using the Speculative RAT.
--   3. Checks for available resources (ROB slots, GPRs, LSU queue space).
--   4. Dispatches instructions to the Reservation Stations if all resources exist.
----------------------------------------------------------------------------------
entity RR_stage is
    Port (
        clk                 : in  std_logic;
        ...
        -- Deallocation Interface (From Commit)
        ...
        prs1_out            : out std_logic_vector(4 downto 0);
        prt1_out            : out std_logic_vector(4 downto 0);
        prs1_rdy            : out std_logic;
        prt1_rdy            : out std_logic;
        pr_flag1_out        : out std_logic_vector(4 downto 0);
        pr_flag1_rdy        : out std_logic;
        pd1_new             : out std_logic_vector(4 downto 0);
        pd1_old             : out std_logic_vector(4 downto 0); 
        pf1_new             : out std_logic_vector(4 downto 0);
        pf1_old             : out std_logic_vector(4 downto 0);
        ...
        prs2_out            : out std_logic_vector(4 downto 0);
        prt2_out            : out std_logic_vector(4 downto 0);
        prs2_rdy            : out std_logic;
        prt2_rdy            : out std_logic;
        pr_flag2_out        : out std_logic_vector(4 downto 0);
        pr_flag2_rdy        : out std_logic;
        pd2_new             : out std_logic_vector(4 downto 0);
        pd2_old             : out std_logic_vector(4 downto 0); 
        pf2_new             : out std_logic_vector(4 downto 0);
        pf2_old             : out std_logic_vector(4 downto 0);
        ...
        -- Queue Pressures
        lq_full             : in std_logic;
        sq_full             : in std_logic;
...
architecture Structural of RR_stage is
    ...
    -- Readiness signals
    signal bt_prs1_rdy, bt_prt1_rdy, bt_pr_flag1_rdy : std_logic;
    signal bt_prs2_rdy, bt_prt2_rdy, bt_pr_flag2_rdy : std_logic;
    signal fwd_prs1_rdy, fwd_prt1_rdy, fwd_pr_flag1_rdy : std_logic;
    signal fwd_prs2_rdy, fwd_prt2_rdy, fwd_pr_flag2_rdy : std_logic;
    
    signal lsu_stall_req : std_logic;
    
begin

    -- Filter out purely invalid or illegal instructions before evaluating resource needs
    ...
    -- Determine hardware resource demands for this cycle
    ...
    -- Block LSU instructions if the memory subsystem queues are saturated
    lsu_stall_req <= '1' when (lq_full = '1' or sq_full = '1') and (is_lsu1_in = '1' or is_lsu2_in = '1') else '0';

    -- Final gatekeeper: Only fire if ALL required resources are available
    process(needed_gprs, avail_gprs, needed_flags, avail_flags, needed_robs, rob_free_count, flush, lsu_stall_req) begin
        if (flush = '0') and (avail_gprs >= needed_gprs) and (avail_flags >= needed_flags) and (rob_free_count >= needed_robs) and (lsu_stall_req = '0') then
            dispatch_fire <= '1';
        else
            dispatch_fire <= '0';
        end if;
    ...
    -- Qualify resource requests so they only pull from the free list on a successful fire
    ...
    -- Direct passthrough for instruction metadata
    ...
    BT_inst : entity work.busy_table port map (
            ...
            prs1_ready => bt_prs1_rdy, prt1_ready => bt_prt1_rdy, pr_flag1_ready => bt_pr_flag1_rdy, prs2_ready => bt_prs2_rdy, prt2_ready => bt_prt2_rdy, pr_flag2_ready => bt_pr_flag2_rdy, 
    ...
    -- Combinational CDB Forwarding (Asynchronous Wakeup)
    -- If a tag is currently being written back on the CDB, bypass the busy table
    -- and instantly signal to the Reservation Station that the data is ready.
    fwd_prs1_rdy <= '1' when (bt_prs1_rdy = '1') or (wb_gpr1_en = '1' and wb_gpr1_tag = prs1_sig) or (wb_gpr2_en = '1' and wb_gpr2_tag = prs1_sig) or (wb_gpr3_en = '1' and wb_gpr3_tag = prs1_sig) else '0';
    fwd_prt1_rdy <= '1' when (bt_prt1_rdy = '1') or (wb_gpr1_en = '1' and wb_gpr1_tag = prt1_sig) or (wb_gpr2_en = '1' and wb_gpr2_tag = prt1_sig) or (wb_gpr3_en = '1' and wb_gpr3_tag = prt1_sig) else '0';
    fwd_pr_flag1_rdy <= '1' when (bt_pr_flag1_rdy = '1') or (wb_f1_en = '1' and wb_f1_tag = pr_flag1_sig) or (wb_f2_en = '1' and wb_f2_tag = pr_flag1_sig) else '0';

    fwd_prs2_rdy <= '1' when (bt_prs2_rdy = '1') or (wb_gpr1_en = '1' and wb_gpr1_tag = prs2_sig) or (wb_gpr2_en = '1' and wb_gpr2_tag = prs2_sig) or (wb_gpr3_en = '1' and wb_gpr3_tag = prs2_sig) else '0';
    fwd_prt2_rdy <= '1' when (bt_prt2_rdy = '1') or (wb_gpr1_en = '1' and wb_gpr1_tag = prt2_sig) or (wb_gpr2_en = '1' and wb_gpr2_tag = prt2_sig) or (wb_gpr3_en = '1' and wb_gpr3_tag = prt2_sig) else '0';
    fwd_pr_flag2_rdy <= '1' when (bt_pr_flag2_rdy = '1') or (wb_f1_en = '1' and wb_f1_tag = pr_flag2_sig) or (wb_f2_en = '1' and wb_f2_tag = pr_flag2_sig) else '0';

    -- Physical mappings passed forward to the reservation stations
    prs1_out <= prs1_sig; 
    ...
    -- Store Multiple Flag (SMF) Handling: 
    -- Route the Flag physical tag into the PRT port so the Store Queue can save the flag to memory.
    prt1_out <= pr_flag1_sig when (is_store1_in = '1' and cz_bits1_in = "11") else prt1_sig;
    prt2_out <= pr_flag2_sig when (is_store2_in = '1' and cz_bits2_in = "11") else prt2_sig;

    prs1_rdy <= fwd_prs1_rdy;
    prs2_rdy <= fwd_prs2_rdy;

    -- Mirror readiness checking for the SMF routing mechanism
    prt1_rdy <= fwd_pr_flag1_rdy when (is_store1_in = '1' and cz_bits1_in = "11") else fwd_prt1_rdy;
    prt2_rdy <= fwd_pr_flag2_rdy when (is_store2_in = '1' and cz_bits2_in = "11") else fwd_prt2_rdy;
    
    -- Flag Dependency Check (Force ready to '1' if the instruction doesn't need to read the flags)
    pr_flag1_rdy <= fwd_pr_flag1_rdy when (((op1_in = "0001" or op1_in = "0010") and cz_bits1_in /= "00") or cz_bits1_in = "11") else '1';
    pr_flag2_rdy <= fwd_pr_flag2_rdy when (((op2_in = "0001" or op2_in = "0010") and cz_bits2_in /= "00") or cz_bits2_in = "11") else '1';

    -- Logging process for simulation tracking
    process(clk)
        variable l : line;
    begin
        if rising_edge(clk) then
            if rst = '0' then
                if flush = '1' then
                    write(l, string'("[RR STAGE] FLUSHED -> Wiping Allocations")); writeline(output, l);
                elsif dispatch_fire = '0' and (real_valid_1 = '1' or real_valid_2 = '1') then
                    write(l, string'("[RR STAGE] STALLED -> Need GPRs: ")); write(l, integer'image(to_integer(needed_gprs)));
                    write(l, string'(" | Avail GPRs: ")); write(l, integer'image(to_integer(avail_gprs)));
                    write(l, string'(" | Need Flags: ")); write(l, integer'image(to_integer(needed_flags)));
                    write(l, string'(" | Avail Flags: ")); write(l, integer'image(to_integer(avail_flags)));
                    write(l, string'(" | Free ROBs: ")); write(l, integer'image(to_integer(rob_free_count)));
                    writeline(output, l);
                end if;
            end if;
        end if;
