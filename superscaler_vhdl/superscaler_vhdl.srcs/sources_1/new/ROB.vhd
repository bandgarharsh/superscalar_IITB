...
entity ROB is
    Port (
        ...
        alu_cond_fail       : in std_logic; -- Add this right under alu_flags!
    ...
    begin
        if rising_edge(clk) then
            if rst = '1' then
                ...
                global_flush <= '0'; commit_valid_1 <= '0'; commit_valid_2 <= '0';
                ...
                v_flush := '0'; global_flush <= '0'; commit_valid_1 <= '0'; commit_valid_2 <= '0';
                commit_flag_valid_1 <= '0'; commit_flag_valid_2 <= '0'; commit_store_valid <= '0';
                ...
                if v_rob(v_idx1).valid = '1' and v_rob(v_idx1).executed = '1' then
                    if v_rob(v_idx1).is_branch = '1' and v_rob(v_idx1).mispredicted = '1' then
                        ...
                        if v_rob(v_idx1).we_gpr = '1' then
                            commit_valid_1 <= '1'; commit_arch_dest_1 <= v_rob(v_idx1).arch_dest;
                            commit_pd_1 <= v_rob(v_idx1).pd_new; commit_data_1 <= v_rob(v_idx1).gpr_data; 
                        end if;
                        ...
                        if v_rob(v_idx2).valid = '1' and v_rob(v_idx2).executed = '1' then
                            if v_rob(v_idx2).op_type = "10" and v_store_used then null; 
                                ...
                                if v_rob(v_idx2).we_gpr = '1' then
                                    commit_valid_2 <= '1'; commit_arch_dest_2 <= v_rob(v_idx2).arch_dest;
                                    commit_pd_2 <= v_rob(v_idx2).pd_new; commit_data_2 <= v_rob(v_idx2).gpr_data; 
                                end if;
                ...
                if alu_complete = '1' then
                    ...
                    -- FAKE A BRANCH MISPREDICT IF THE CONDITION FAILED!
                ...
                -- PHASE 3: DISPATCH (Only if NO STALL!)
                if v_flush = '0' and is_full = '0' and dispatch_stall = '0' then
                    if disp_valid_1 = '1' then
                        if disp_packet_1.pc /= last_allocated_pc1 then
                            ...
                            v_rob(to_integer(v_tail)).valid := '1'; v_rob(to_integer(v_tail)).executed := '0'; 
                            v_rob(to_integer(v_tail)).pc := disp_packet_1.pc; v_rob(to_integer(v_tail)).pd_new := disp_packet_1.pd_new;
                            v_rob(to_integer(v_tail)).arch_dest := disp_packet_1.arch_dest; v_rob(to_integer(v_tail)).we_gpr := disp_packet_1.we_gpr;
                            v_rob(to_integer(v_tail)).we_flag := disp_packet_1.we_flag; v_rob(to_integer(v_tail)).pf_new := disp_packet_1.pf_new;   
                    ...
                    if disp_valid_2 = '1' then
                        if disp_packet_2.pc /= last_allocated_pc2 then
                            ...
                            v_rob(to_integer(v_tail)).valid := '1'; v_rob(to_integer(v_tail)).executed := '0'; 
                            v_rob(to_integer(v_tail)).pc := disp_packet_2.pc; v_rob(to_integer(v_tail)).pd_new := disp_packet_2.pd_new;
                            v_rob(to_integer(v_tail)).arch_dest := disp_packet_2.arch_dest; v_rob(to_integer(v_tail)).we_gpr := disp_packet_2.we_gpr;
                            v_rob(to_integer(v_tail)).we_flag := disp_packet_2.we_flag; v_rob(to_integer(v_tail)).pf_new := disp_packet_2.pf_new;   
