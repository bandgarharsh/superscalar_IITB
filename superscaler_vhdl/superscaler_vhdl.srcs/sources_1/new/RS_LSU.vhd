            ...
            if rst = '1' or flush = '1' then
                ...
                for offset in 0 to 3 loop
                    ...
                    if not v_issued and v_rs(v_idx).valid = '1' then
                        -- For memory, we ONLY need the Base Register (prs_rdy) to execute!
                        if v_rs(v_idx).prs_rdy = '1' then
                            
                            if v_rs(v_idx).op = OP_SW then v_is_store := '1'; else v_is_store := '0'; end if;
                            ...
                            issue_packet.base_data  <= v_rs(v_idx).prs_data;
                            issue_packet.prt_tag    <= v_rs(v_idx).prt_tag;
                            issue_packet.prt_rdy    <= v_rs(v_idx).prt_rdy;
                            issue_packet.prt_data   <= v_rs(v_idx).prt_data;
                ...
                -- -------------------------------------------------------------
                -- PHASE 3: WAKEUP / SNOOP CDB (GPR Only)
                -- -------------------------------------------------------------
                for i in 0 to 3 loop
