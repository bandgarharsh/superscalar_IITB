...
architecture Behavioral of ID_stage is
    ...
    -- Internal Flag Signals for ORing
    signal writes_c1_s, writes_z1_s : std_logic;
    signal writes_c2_s, writes_z2_s : std_logic;

begin
    -- =================================================================
    -- X-RAY 1: WHAT IS THE DECODER EXTRACTING?
    -- =================================================================
    process(clk)
    begin
        if rising_edge(clk) then
            if dec_valid1 = '1' then
                report "[X-RAY DECODE 1] PC: " & integer'image(to_integer(unsigned(pc1_in))) & 
                       " | OP: " & integer'image(to_integer(unsigned(op1_s))) &
                       " | RS1: " & integer'image(to_integer(unsigned(src1_reg1_s))) &
                       " | RS2: " & integer'image(to_integer(unsigned(src2_reg1_s))) &
                       " | DEST: " & integer'image(to_integer(unsigned(dest_reg1_s))) &
                       " | IMM: " & integer'image(to_integer(unsigned(imm1_s)));
            end if;
            if dec_valid2 = '1' then
                report "[X-RAY DECODE 2] PC: " & integer'image(to_integer(unsigned(pc2_in))) & 
                       " | OP: " & integer'image(to_integer(unsigned(op2_s))) &
                       " | RS1: " & integer'image(to_integer(unsigned(src1_reg2_s))) &
                       " | RS2: " & integer'image(to_integer(unsigned(src2_reg2_s))) &
                       " | DEST: " & integer'image(to_integer(unsigned(dest_reg2_s))) &
                       " | IMM: " & integer'image(to_integer(unsigned(imm2_s)));
            end if;
        end if;
    end process;
    ------------------------------------------------------------------
    ...
    -- Generate Flag Write Enable for the Rename Stage
    -- Only ADD, ADC, ADZ, AWC, ACA, ACC, ACZ, ACW, ADI, NDU, NDC, NDZ, NCU, NCC, NCZ write to flags.
    -- (In our decoder, we marked these as 'is_alu', except LLI which is also 'is_alu' but doesn't write flags).
